...
async def predict(file: UploadFile = File(...)) -> dict:
    ...
    **Error codes**
    ...
    # ── 8.5. Detect Healthy Skin / No Disease ──────────────────────────────
    # Our model is forced to pick a disease class. If the highest confidence
    # is under 60%, it's highly likely to be healthy skin or an unknown object.
    if top_confidence < 60.0:
        return {
            "success": True,
            "top_prediction": {
                "disease":        "healthy_skin",
                "disease_label":  "Skin Disease Not Detected",
                "confidence":     round(100.0 - (top_confidence / 2.0), 2),
                "severity":       "low",
                "severity_label": "Healthy or undetectable",
                "severity_color": "#22c55e",
                "description":    "The AI did not detect clear visual evidence of any specific skin disease in this image. This usually indicates healthy skin.",
                "symptoms":       ["Clear skin texture", "No obvious lesions, scaling, or rashes"],
                "causes":         ["Healthy biology and good skin care"],
                "urgency":        "None",
                "visual_analysis": "The visual patterns do not strongly align with any known disease profiles in our database.",
                "confidence_warning": (
                    "The model found a very slight resemblance to "
                    f"{get_friendly_name(predictions[0]['disease'])} ({top_confidence:.1f}%), "
                    "but this is too low to be considered a positive detection."
                ),
            },
            "all_predictions": [
                {
    ...
    # We no longer issue a soft warning here because anything under 60%
    # is now handled by the Healthy Skin block above.
            ...
            "doctors": [
                {
                    ...
                    "total_ratings": 84,
                    "open_now":      True,
                    "distance_km":   max(0.5, round(radius / 1000 * 0.2, 1)),
                    "maps_url":      f"https://www.google.com/maps/search/?api=1&query={lat},{lng}",
                },
                {
                    "name":          "City Dermatology Center",
                    "address":       "Downtown Medical Wing",
                    "rating":        4.5,
                    "total_ratings": 210,
                    "open_now":      True,
                    "distance_km":   max(1.2, round(radius / 1000 * 0.4, 1)),
                    "maps_url":      f"https://www.google.com/maps/search/?api=1&query={lat},{lng}",
                },
                {
                    "name":          "Advanced Skin & Laser",
                    "address":       "Health Avenue",
                    "rating":        4.2,
                    "total_ratings": 56,
                    "open_now":      False,
                    "distance_km":   max(2.5, round(radius / 1000 * 0.8, 1)),
                    "maps_url":      f"https://www.google.com/maps/search/?api=1&query={lat},{lng}",
                }
            ],
